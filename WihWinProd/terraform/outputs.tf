...
output "ssh_command" {
  ...
  value       = "ssh -i ~/.ssh/wihwin-key ec2-user@${aws_instance.wihwin.public_ip}"
}

output "deploy_command" {
  description = "Command to trigger deployment on EC2"
  value       = "ssh -i ~/.ssh/wihwin-key ec2-user@${aws_instance.wihwin.public_ip} 'sudo /opt/${var.project_name}/deploy.sh'"
}
...
output "key_pair_name" {
  ...
  value       = aws_key_pair.wihwin.key_name
}
