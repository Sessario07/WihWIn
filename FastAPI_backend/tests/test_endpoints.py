...
# Patch database pool before importing app
with patch('config.database.init_pool', new_callable=AsyncMock), \
     patch('services.ride_service.init_rabbitmq', new_callable=AsyncMock), \
     patch('config.database.close_pool', new_callable=AsyncMock), \
     patch('services.ride_service.close_rabbitmq', new_callable=AsyncMock):
...
class TestRideStartEndpoint:
    ...
    def test_start_ride_success(self, mock_ride_repo, mock_device_repo):
        ...
        mock_device_repo.get_device_by_id = AsyncMock(return_value={
            'id': device_uuid,
            'device_id': 'TEST-DEVICE-001',
            'user_id': str(uuid.uuid4())
        })
        mock_ride_repo.get_active_ride = AsyncMock(return_value=None)
        mock_ride_repo.create_ride = AsyncMock(return_value=ride_uuid)
    ...
    def test_start_ride_device_not_found(self, mock_device_repo):
        ...
        mock_device_repo.get_device_by_id = AsyncMock(return_value=None)
    ...
    def test_start_ride_already_active(self, mock_ride_repo, mock_device_repo):
        ...
        mock_ride_repo.get_active_ride = AsyncMock(return_value={'id': existing_ride_id})
...
class TestRideEndEndpoint:
    ...
    @patch('services.ride_service.publish_ride_end', new_callable=AsyncMock)
    @patch('services.ride_service.RideRepository')
    def test_end_ride_success(self, mock_ride_repo, mock_publish):

        ride_id = str(uuid.uuid4())
        mock_ride_repo.mark_ride_ending = AsyncMock(return_value=True)
    ...
    def test_end_ride_not_found(self, mock_ride_repo):
        ...
        ride_id = str(uuid.uuid4())
        mock_ride_repo.mark_ride_ending = AsyncMock(return_value=False)
        mock_ride_repo.get_ride_by_id = AsyncMock(return_value=None)
    ...
    def test_end_ride_already_completed(self, mock_ride_repo):
        ...
        ride_id = str(uuid.uuid4())
        mock_ride_repo.mark_ride_ending = AsyncMock(return_value=False)
        mock_ride_repo.get_ride_by_id = AsyncMock(return_value={
            'id': ride_id,
            'status': 'completed'
        })
...
class TestCrashEndpoint:
    ...
    def test_crash_alert_with_hospital(self, mock_device_repo, mock_user_repo):
        ...
        mock_user_repo.find_nearest_hospital = AsyncMock(return_value={
            'id': doctor_uuid,
            'hospital_name': 'Test Hospital',
            'distance_km': 2.5
        })
        mock_user_repo.get_user_info = AsyncMock(return_value={
            'username': 'testuser',
        ...
        mock_user_repo.create_crash_alert = AsyncMock(return_value=str(uuid.uuid4()))
    ...
    def test_crash_alert_no_hospital_nearby(self, mock_device_repo, mock_user_repo):
        ...
        mock_user_repo.find_nearest_hospital = AsyncMock(return_value=None)
        mock_user_repo.create_crash_alert = AsyncMock(return_value=str(uuid.uuid4()))
...
class TestBaselineEndpoint:
    ...
    def test_compute_baseline_success(self, mock_nk, mock_device_repo, mock_baseline_repo):
        ...
        mock_device_repo.get_device_by_id = AsyncMock(return_value={'id': device_uuid})
        mock_baseline_repo.save_baseline = AsyncMock(return_value=None)
        mock_device_repo.mark_onboarded = AsyncMock(return_value=None)
