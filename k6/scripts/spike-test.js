...
function generateUserCredentials(index) {
    ...
    return {
        username: `spike_user_${index}_${timestamp}_${random}`,
        email: `spike${index}_${timestamp}_${random}@example.com`,
        password: `SpikePass123!${index}`,
        role: 'customer',
    };
}
...
export function setup() {
    ...
    if (users.length === 0) {
        throw new Error('No users registered successfully. Cannot proceed with spike test.');
    }
...
export function testSpringLogin(data) {
    ...
    const loginSuccess = check(loginRes, {
        'Spring login responded': (r) => r.status !== 0,
...
export function testFastApiBatch() {
    ...
    const batchSuccess = check(batchRes, {
        'FastAPI batch responded': (r) => r.status !== 0,
        'FastAPI batch status is 200': (r) => r.status === 200,
    });
    
    if (batchSuccess && batchRes.status === 200) {
...
export function handleSummary(data) {
    ...
    const springRps = springTotal > 0 ? (springTotal / 90).toFixed(2) : 0;
    ...
    const fastApiRps = fastApiTotal > 0 ? (fastApiTotal / 90).toFixed(2) : 0;
    
    console.log('\n' + '='.repeat(70));
    console.log('                    SPIKE TEST RESULTS');
    console.log('='.repeat(70));
    console.log(`Test Type:              Spike Test (Immediate Traffic Spike)`);
    ...
    console.log('\nSPRING BOOT LOGIN ENDPOINT (SPIKE):');
    console.log('-'.repeat(70));
    console.log(`  Target Spike RPS:     500 (immediate)`);
    ...
    console.log('\nFASTAPI BATCH TELEMETRY ENDPOINT (SPIKE):');
    console.log('-'.repeat(70));
    console.log(`  Target Spike RPS:     1000 (immediate)`);
