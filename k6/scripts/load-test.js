...
let registeredUsers = new SharedArray('registeredUsers', function() {
    return [];
});
...
export const options = {
    ...
    thresholds: {
        ...
        spring_login_latency: ['p(95)<300'],
        fastapi_batch_latency: ['p(95)<400'],
    },
};
...
function generateUserCredentials(index) {
    ...
    return {
        username: `loadtest_user_${index}_${timestamp}_${random}`,
        email: `loadtest${index}_${timestamp}_${random}@example.com`,
...
function registerUser(credentials) {
    ...
    console.error(`Registration failed for ${credentials.username}: Status ${response.status}, Body: ${response.body}`);
    return null;
}

export function setup() {
    console.log('\n' + '='.repeat(60));
    console.log('SETUP PHASE: Registering test users...');
    ...
    for (let i = 0; i < REGISTRATION_COUNT; i++) {
        ...
        if (registeredUser) {
            ...
            failCount++;
            console.log(`Failed to register user ${i + 1}`);
        }
    ...
    if (users.length === 0) {
        throw new Error('No users registered successfully. Cannot proceed with load test.');
    }
...
export function handleSummary(data) {
    ...
    const springRps = (springTotal / 90).toFixed(2);
    ...
    const fastApiRps = (fastApiTotal / 90).toFixed(2);
    
    console.log('\n' + '='.repeat(70));
    console.log('                    LOAD TEST RESULTS');
    console.log('='.repeat(70));
    console.log(`Test Duration:          90s (1m 30s)`);
    ...
    console.log(`  Actual RPS:           ${springRps}`);
    console.log(`  Total Requests:       ${springTotal}`);
    console.log(`  Successful:           ${springSuccess} (${((springSuccess/springTotal)*100).toFixed(2)}%)`);
    console.log(`  Failed:               ${springFailed} (${((springFailed/springTotal)*100).toFixed(2)}%)`);
    console.log(`  Avg Latency:          ${springAvg}ms`);
    console.log(`  P95 Latency:          ${springP95}ms`);
    
    console.log('\nFASTAPI BATCH TELEMETRY ENDPOINT:');
    console.log('-'.repeat(70));
    console.log(`  Target RPS:           200`);
    console.log(`  Actual RPS:           ${fastApiRps}`);
    console.log(`  Total Requests:       ${fastApiTotal}`);
    console.log(`  Successful:           ${fastApiSuccess} (${((fastApiSuccess/fastApiTotal)*100).toFixed(2)}%)`);
    console.log(`  Failed:               ${fastApiFailed} (${((fastApiFailed/fastApiTotal)*100).toFixed(2)}%)`);
    ...
    console.log(`  Flush cycles:         ${(90 / 120).toFixed(2)}`);
    console.log('='.repeat(70) + '\n');
